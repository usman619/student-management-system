...
<head>
    ...
    <?php
                ...
                $query_1 = $conn->prepare("Select * from Student;"); 
                $query_1->execute();
...
<h2>Faculty Information</h2>
            ...
            foreach ($result as $value){
                ...
                $query_1 = $conn->prepare("select * from student NATURAL JOIN login where username = ?"); 
                $query_1->execute([$user_name]);
                $result   = $query_1->fetchAll(PDO::FETCH_ASSOC);
                
                $query_2 = $conn->prepare("select GPA from course_marks where USERNAME = ?;"); 
                $query_2->execute([$user_name]);
                $GPA = $query_2->fetchAll(PDO::FETCH_ASSOC);

                $query_3 = $conn->prepare("select CR_HOURS from course_info NATURAL JOIN course_marks where USERNAME = ?;"); 
                $query_3->execute([$user_name]);
                $CR_HOURS = $query_3->fetchAll(PDO::FETCH_ASSOC);
                // echo '<br>';
                // echo "CR_HOURS: " .$CR_HOURS[2]['CR_HOURS'];
                $i = 0;
                $T = 0;
                $CR = 0;
                foreach($GPA as $temp)
                {
                    $T += $temp['GPA'] * $CR_HOURS[$i]['CR_HOURS'];
                    $CR += $CR_HOURS[$i]['CR_HOURS'];
                    $i++;
                    // echo "T: " .$T;
                    // echo "<br>";
                    // echo "GT: " .$CR;
